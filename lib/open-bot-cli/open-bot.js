	...
	loadConfig()
		...
		.then(config => {
			...
			return Promise.all(items.map(item => {
				...
				.then(workItems => {
					const { reporter, finish } = createReporter();
...
function createReporter() {
	...
	let actions = [];
	let errors = [];
	const reporter = ({ item, action, change, message, error, stack }) => {
		if(error) {
			errors.push({
				item,
				error,
			...
			let line = `${queuedJobs} queued, ${activeJobs} active, ${completedJobs} completed, ${item} ${action}`;
