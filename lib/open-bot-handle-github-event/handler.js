...
module.exports = (event, context, callback) => {
	...
	if(typeof number !== "number" && typeof sha === "string") {
		...
		github.getPullRequestsForCommit(owner, repo, sha).then(prs => {
			if(!prs || prs.length === 0) {
				console.log("Commit is not part of a PR");
				return callback(new Error("[200] commit has no PR"));
			}
			const pr = prs[0];
	...
	function publish(number) {
		...
		const params = {
			...
			TopicArn: `arn:aws:sns:us-east-1:${config.awsAccountId}:open-bot-${process.env.STAGE}-processIssue`,
		};
